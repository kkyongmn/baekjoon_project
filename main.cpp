    ...
    for (int rowOffset = -1; rowOffset <= 1; ++rowOffset) {
        for (int colOffset = -1; colOffset <= 1; ++colOffset) {
            ...
            if (currentRow >= 0 && currentRow < numRows && currentCol >= 0 && currentCol < numCols) {
                ...
                if (!(rowOffset == 0 && colOffset == 0) && (pixels[currentRow][currentCol].red != 0 || 
                    pixels[currentRow][currentCol].green != 0 || pixels[currentRow][currentCol].blue != 0)) {
...
int main() {
    ...
    std::cout << "Initial image has been written to " << initialFileName << ".\n";
    ...
    for (int step = 1; step <= 20; ++step) {
        ...
        // 수정된 이미지를 새로운 PNG 파일로 저장
        std::string stepFileName = "modified_image_step" + std::to_string(step) + ".png";
        writePNG(stepFileName.c_str());
        std::cout << "Step " << step << " image has been written to " << stepFileName << ".\n";
    }
